...
CONFIRM_RULES: tuple[Rule, ...] = (
    ...
    Rule(
        "confirm_required",
        re.compile(r"\b(apt|apt-get|yum|dnf|pacman|zypper)\b.*\b(install|remove|upgrade|downgrade|purge|autoremove|reinstall)\b"),
    ...
    Rule(
        "confirm_required",
        re.compile(r"\b(sysctl)\b.*\s-w\s+|(^|[;&|])\s*echo\s+.+>\s*/proc/sys/"),
        "Runtime kernel parameter mutation.",
        "Can change network, memory, or security behavior immediately.",
        "Restore previous sysctl values and verify runtime state.",
        "reversible_change",
    ),
    Rule(
        "confirm_required",
        re.compile(r"\b(insmod|rmmod|modprobe)\b"),
        "Kernel module load/unload mutation.",
        "Can destabilize host behavior or alter investigation visibility.",
        "Restore known-good module set and reboot only under change control.",
        "business_interruption",
    ),
    Rule(
        "confirm_required",
        re.compile(r"\b(docker|podman)\b\s+(stop|rm|kill|restart|update|rename)\b"),
        "Container lifecycle mutation.",
        "Can interrupt business services or destroy volatile runtime evidence.",
        "Restore container state from orchestration or image definition.",
        "business_interruption",
    ),
    Rule(
        "confirm_required",
        re.compile(r"\bkubectl\b\s+(delete|apply|replace|patch|scale)\b|\bkubectl\b.*\brollout\s+restart\b"),
        "Cluster workload mutation.",
        "Can change production workload state or remove evidence.",
        "Restore workload definition from version-controlled manifests.",
        "business_interruption",
    ),
    Rule(
        "confirm_required",
